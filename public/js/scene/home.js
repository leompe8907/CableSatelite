	...
	$.extend(true, Scene_Home.prototype, Scene.prototype, {
		...
		onFocus: function ($el) {
			...
			if (typeof $el.data("type") !== 'undefined' && typeof $el.data("id") !== 'undefined' && $el.data("id") != null) {
				...
				$(".nb-vjs-tooltip").hide();
				if (Focus.focused != this.$videoContainer && Focus.focused.find(".nb-vjs-tooltip").length > 0) {
					Focus.focused.find(".nb-vjs-tooltip").show();
				}
			}
        },
		/**
		...
		onEnter: function($el, event) {
			...
			if (nbPlayer.isFullscreen()) {
				...
				EPG.onEnter($el, function(type, id, url, object) {
					self.playContentWithAccess(type, id, url, object, false, true);
				});
				return;
			...
			if (typeof $el.data("id") !== 'undefined') {
				...
				if (type == "service") {
					...
					if (serviceTV !== false && serviceTV.url != null && serviceTV.url.length > 0) {
						this.playContentWithAccess(type, id, serviceTV.url, serviceTV, false, false);
					}
					...
					AppData.getTopLevelCatchupM3u8Url(catchup.id, function(url) {
						...
						if (url != null && url.length > 0) {
							self.playContentWithAccess(type, id, url, catchup, false, false);
						}
					});
		...
		openCatchupDate: function(catchup, dateString) {
			...
			var cells = '<div class="channel-video focusable" data-id="' + catchup.epgStreamId + '" data-type="catchup" data-back="true" style="' + style + '">'
					...
					+ '</div>';
					var config = localStorage.getItem("cvClientConfig");
		...
		getHTMLRowVOD: function(vods, title, idCategory, allOption) {
			...
			var config = localStorage.getItem("base_url");
		...
		playContent: function(type, id, url, item, reset) {
			...
			if (type == "service") { //live
				...
				if (liveEvent != null) {
					...
					epgNowText = (liveEvent.languages.length > 0 ? liveEvent.languages[0].title : "");
					playerTime1Text = getStringDate(liveEvent.startDate, "HH:mm") + " - " + getStringDate(liveEvent.endDate, "HH:mm");
					epgNextText = (nextEvent.languages.length > 0 ? nextEvent.languages[0].title : "");
				...
				srcPlaceholder = null;
				nbPlayer.$itemImageDiv.attr("style", "visibility: hidden");
			}
			...
			if (nbPlayer.nbPlayerAreControslActive()) {
		...
		playServiceTVByChannel: function(newChannel) {
			if (typeof newChannel !== 'undefined' && newChannel != null) {
				...
				this.playContentWithAccess("service", newChannel.id, newChannel.url, newChannel);
				// if (nbPlayer.isFullscreen()) {
				// 	setTimeout(function() {
				// 		nbPlayer.showControls();
				// 	}, 300);
		...
		playNextCatchup: function(currentEventId) {
			...
			if (catchup && catchup != null) {
				AppData.getTopLevelCatchupM3u8Url(catchup.id, function(url) {
					console.log("Play CATCHUP with URL: " + url);
					if (url != null && url.length > 0) {
						self.playContentWithAccess("catchup-event", catchup.eventId, url, catchup, false, false);
					} else {
						nbPlayer.nbPlayerResetContent(true);
		...
		checkParentalControl: function(type, id, url, item, reset, forceFullscreen) {
			var self = this;
			if (type == "service") {
				if (typeof item.parentalControl != 'undefined' && item.parentalControl) {
					// requiere pin
					this.$lastFocused = Focus.focused;
					...
					if (nbPlayer.isFullscreen()) {
						...
						nbPlayer.exitFullscreen(function() {
							self.showAlertPin(type, id, url, item, reset, forceFullscreen);
						});
					} else {
                        this.showAlertPin(type, id, url, item, reset, forceFullscreen);
                    }
					return;
				}
			} 
			
			this.playContent(type, id, url, item, reset);
			if (forceFullscreen) {
				nbPlayer.requestFullscreen();
			}
		},

		showAlertPin: function(type, id, url, item, reset, forceFullscreen) {
            var self = this;
			NBAlert.showAlertInput(this, __("ServicesAndTVEnterParentalControlPin"), "parentalControl", __("SettingsOkButton").toUpperCase(), null, null, true, function () {
				var pin = NBAlert.getInputText();
				self.validatePin(pin, type, id, url, item, reset, forceFullscreen);
			}, null);
		},

		validatePin: function(pin, type, id, url, item, reset, forceFullscreen) {
			var self = this;
			NBAlert.close();
			...
			if (pin == User.getLicensePin()) {	
				...
				setTimeout(function() {
					self.checkParentalControl(type, id, url, item, reset, forceFullscreen);
				}, 500)
			}
		}
