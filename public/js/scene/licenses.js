    ...
    $.extend(true, Scene_Licenses.prototype, Scene.prototype, {
		...
		navigate: function(direction) {
            ...
            if ($el.isInAlertMessage(this.$el) || $el.isInAlertConfirm(this.$el)) { // navigate on dialog
                ...
                if (direction === 'down') {
                    ...
                    Focus.to($newFocus);
                    if ($newFocus.offset().top + $newFocus.height() + 50 > this.viewport.height()) {
                        this.viewport.animate({ scrollTop: this.viewport.scrollTop() + $newFocus.height() + 50 - (this.viewport.height() - $newFocus.offset().top) }, 200);
                    }
                    ...
                    if (typeof $newFocus === 'undefined' || $newFocus.length == 0) {
                        this.viewport.animate({ scrollTop: 0 }, 200);
                        return;
                    }
        ...
        setLicensesData: function() {
            ...
            this.licenses.forEach(function(license, index, array) {
                ...
                + '<b>' + license.key + '</b><p class="font-weight-normal">' + license.products + '</p>'
                + '</button>';
            });
