jQuery.fn.extend({
    ...
    nbDropdown: function(options, current) {
        ...
        options.forEach(function(option) {
            items += "<a href='javascript:;' data-id='" + option.id + "' class='focusable'>" + option.description + "</a>";  
        });
        
        var currentId = current < options.length ? options[current].id : "";
        var currentDescription = current < options.length ? options[current].description : "";
